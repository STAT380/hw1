...
This vector contains string values. although they look like numbers, the
quotations make them strings.
...
1.  Create a new vector `my_vec_bool` which comprises of:
    ...
    How many elements of `my_vec_double` are greater than zero? 4
    elements

2.  Sort the values of `my_vec_double` in ascending order.

```{R}
sort(my_vec_double, decreasing = F)
```
...
v1 <- c(1:100)
v2 <- c((v1)^2)
mat2 <- matrix(c(v1, v2), nrow = 2, byrow = T)
mat2
```
...
function here[footnote]

4.  Between `col_wise_scan` and `row_wise_scan`, which function do you
    expect to take shorter to run? Why? I predict col_wise_scan to take
    shorter to run because of the way the computer stores data.
...
Which took longer to run? row_wise_scan took longer to run than
col_wise_scan as i predicted.
...
What can you conclude? In each of these examples, the row_wise_scan
always took longer to run. As the matrix got larger the time for both
increased as well.
...
[footnote]: If your code is right, the following code should evaluate to be
    `TRUE`
