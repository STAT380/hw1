...
```{R}
# I believe the two functions have about the same running time, as they both check each individual element to determine whether it is greater than or equal to zero. Whether it checks columns or rows first makes no difference in this case.
```
...
```{R}
# They are about the same; when the above code chunk is run multiple times, the row_wise_time function runs longer sometimes, while the col_wise_time function runs longer other times.
```
...
```{R}
# The size of the matrix makes no impact on which function is faster, it only increases the time needed for both functions to run.
```
<br><br><br><br>
<br><br><br><br>
---
