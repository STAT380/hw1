...
2.  Clone your Github repository on your local machine
    ...
    ``` bash
    $ git clone <<https://github.com/pzp5254/hw1-paragpatel.git>>
    $ cd hw-1
    ```
...
3.  Create a new vector `my_vec_bool` which comprises of:
    ...
    ```{R}
    print("There are 4 elements of 'my_vec_double' are greater than zero.")
    ```
...
```{R}
print("The function 'col_wise_scan' would take shorter to run because the way it runs R's column order of storing matrix data")
```
...
```{R}
print("row_wise took longer to run")
```
...
```{R}
print("For M is a 100 x 100 matrix the time is basically the same. The higher number yuou go for the matrix the more time is takes to run. It also takes longer for row_wise than col_wise.")
```
