...
5.  Write a function `time_scan` which takes in a method `f` and a
    matrix `M` and outputs the amount of time taken to run `f(M)`. Which took longer to run?
...
```
    ```
    The row wise increases much faster than column wise does as matrix size increases.
```{R}
