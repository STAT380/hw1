...
#using typeof function to answer question 1.
typeof(my_vec)
...
typeof(my_vec_double) #printing type to show it is double

#changing type to integer
my_vec_int <- as.integer(my_vec)
typeof(my_vec_int) #printing type to show it is integer
...
#new vector with boolean values that determine elements that are >= and <= zero from my_vec_double
my_vec_bool <- (my_vec_double <= 0) 
#printing vector to make sure it works
my_vec_bool 

#counting FALSE elements to find out how many elements of my_vec_double are > zero
#FALSE elements are counted as zeros, so I must subtract the TRUE elements from the total length of my_vec_bool
...
#using sort function to answer question 4
sort(my_vec_double)
...
#matrix with values representing the squares
...
```{R}
M <- generate_matrix(5000) #changing argument for question6
mean(M)
```
...
col_wise_scan <- function(x){
    ...
    #Same code just flip which loop is first to be able to scan columns before row.
...
#Used code from footnote to see if it works
col_wise_scan(M)
```
...
```{r}
print("Since matrices are stored in column major order in R, I believe that the col_wise_scan will take shorter to run, even though it all depends on the size of the matix.")
...
```{r}
print("row_wise_scan took longer to run as seen buy the time_scan function")
```
...
```{r}
print("When M = 100, 1000, and 5000 matrix the time increased for both functions, but col_wise_scan is still faster")
print("I can conclude that col_wise_scan is the fucntion that will iterate through the matix in the shortest time")
