...
2.  Clone your Github repository on your local machine
    ...
    ``` bash
    $ git clone <<https://github.com/kbm5999/hw1KM.git>>
    $ cd hw-1
    ```
...
```{R}
MatrixA <- matrix(data=1:100, nrow=1, ncol=100, byrow = TRUE)
X<- c(1:100)
rbind(MatrixA, X^2)
```
...
4.  Between `col_wise_scan` and `row_wise_scan`, which function do you
    ...
    I expect the column function to take shorter to run because the
    function is built within r, which by default works column by column.
...
The row_wise_time function took longer to run.
...
I can conclude that the larger the matrix the longer the code take to
run, hence the 5000 x 5000 matrix taking up to 3 seconds, which is too
be expected since there are more elements to work with.
