...
2.  Create two new vectors called `my_vec_double` and `my_vec_int` which
    ...
    my_vec_int <- as.integer(round(as.numeric(my_vec)))
    print(my_vec_double)
    print(my_vec_int)
    ```
...
matrix2 <- matrix(c(1:100, (1:100)^2), nrow=2, ncol=100)
print(matrix1)
#print(matrix2)
```
...
4.  Between `col_wise_scan` and `row_wise_scan`, which function do you
    ...
    I would expect the col_wise_scan function to take shorter to run
    because it accesses matrix elements in the order they are stored in
    memory (column by column). In R, the default storage for matrices is
    in column-major order.

5.  Write a function time_scan which takes in a method f and a matrix M
    and outputs the amount of time taken to run `f(M)`
...
The row_wise_time matrix took longer to run.
...
The time taken by both function will obviously increase but the rate of
increase will differ between the two functions. Also the computational
complexity is worth noting. Both functions have a computational
complexity of O(n\^2) for an n x n matrix, as they scan each element
once. However, the actual run time will be influenced by factors other
than just computational complexity.
