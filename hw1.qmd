...
2.  Clone your Github repository on your local machine
    ...
    ``` bash
    $ git clone <<https://github.com/madddwillson/hw1_mxw5663.git
    >>
    $ cd hw-1
    ```
...
1.  What data type does the vector contain?
    ...
    class(my_vec)
    ```
It contains string data values.
...
3.  
4.  Similarly, write a function `col_wise_scan` which does exactly the
...
I expect col_wise_scan to be shorter because matrices are stored in a column major order.


4.  Write a function `time_scan` which takes in a method `f` and a
...
```{R}
# Define matrix sizes
matrix_sizes <- c(100, 1000, 5000)

# Initialize results list
results <- list()

# Run experiment for each matrix size
for (size in matrix_sizes) {
    M <- generate_matrix(size)
    row_wise_time <- time_scan(row_wise_scan, M)
    col_wise_time <- time_scan(col_wise_scan, M)
    
    results[[paste(size, "x", size)]] <- list(
...
print(results)
```
We can conclude that the time differnece increases with size of the matrix. Additionally, row wise increases much faster than column wise does.
