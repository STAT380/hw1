...
2.  Clone your Github repository on your local machine
    ...
    ``` bash
    $ git clone <<https://github.com/dylancrothers/hw1.git>>
    $ cd hw-1
    ```
...
3.  Create a new vector `my_vec_bool` which comprises of:
    ...
    ```{R}
    print("There are 4 elements that are greater than zero.")
    ```
...
```{R}
print("The function 'col_wise_scan' would take shorter to run because of the way it traverses through the matrix.")
```
...
```{R}
print("row_wise_scan took longer to run")
```
...
```{R}
print("For a larger matrix, the time of each scan gets longer. It also creates a larger difference between the column_wise_scan and the row_wise_scane")
```
