...
class(my_vec)
```
This vector contains string values. although they look like numbers, the quotations make them strings. 
...
I predict col_wise_scan to take shorter to run because of the way the computer stores data. 
...
row_wise_scan took longer to run than col_wise_scan as i predicted.
...
In each of these examples, the row_wise_scan always took longer to run. As the matrix got larger the time for both increased as well. 
